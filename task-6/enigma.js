const readline = require('readline');

const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
function mod(n, m) {
  return ((n % m) + m) % m;
}

const ROTORS = [
  { wiring: 'EKMFLGDQVZNTOWYHXUSPAIBRCJ', notch: 'Q' }, // Rotor I
  { wiring: 'AJDKSIRUXBLHWTMCQGZNPYFVOE', notch: 'E' }, // Rotor II
  { wiring: 'BDFHJLCPRTXVZNYEIWGAKMUSQO', notch: 'V' }, // Rotor III
];
const REFLECTOR = 'YRUHQSLDPXNGOKMIEBFZCWVJAT';

function plugboardSwap(c, pairs) {
  for (const [a, b] of pairs) {
    if (c === a) return b;
    if (c === b) return a;
  }
  return c;
}

class Rotor {
  constructor(wiring, notch, ringSetting = 0, position = 0) {
    this.wiring = wiring;
    this.notch = notch;
    this.ringSetting = ringSetting;
    this.position = position;
  }
  step() {
    this.position = mod(this.position + 1, 26);
  }
  atNotch() {
    return alphabet[this.position] === this.notch;
  }
  forward(c) {
    const pos = alphabet.indexOf(c);
    if (pos === -1) return c;
    const shift = mod(pos + this.position - this.ringSetting, 26);
    return this.wiring[shift];
  }
  backward(c) {
    const idx = this.wiring.indexOf(c);
    if (idx === -1) return c;
    const pos = mod(idx - this.position + this.ringSetting, 26);
    return alphabet[pos];
  }
}

class Enigma {
  constructor(rotorIDs, rotorPositions, ringSettings, plugboardPairs) {
    this.rotorIDs = rotorIDs;
    this.initialPositions = rotorPositions;
    this.ringSettings = ringSettings;
    this.plugboardPairs = plugboardPairs;
    this.reset();
  }

  reset() {
    this.rotors = this.rotorIDs.map(
      (id, i) =>
        new Rotor(
          ROTORS[id].wiring,
          ROTORS[id].notch,
          this.ringSettings[i],
          this.initialPositions[i],
        ),
    );
    return this;
  }
  stepRotors() {
    // First, determine which rotors will step
    const rightAtNotch = this.rotors[2].atNotch();
    const middleAtNotch = this.rotors[1].atNotch();

    // The right rotor always steps
    // The middle rotor steps when either:
    // - The right rotor is at its notch
    // - The middle rotor is at its notch (double-stepping)
    // The left rotor steps when the middle rotor is at its notch

    if (middleAtNotch) {
      this.rotors[0].step(); // Step left rotor
      this.rotors[1].step(); // Step middle rotor
    } else if (rightAtNotch) {
      this.rotors[1].step(); // Step middle rotor
    }

    this.rotors[2].step(); // Right rotor always steps
  }
  encryptChar(c) {
    if (!alphabet.includes(c)) return c;
    this.stepRotors();
    c = plugboardSwap(c, this.plugboardPairs);
    for (let i = this.rotors.length - 1; i >= 0; i--) {
      c = this.rotors[i].forward(c);
    }

    const reflectorIdx = alphabet.indexOf(c);
    if (reflectorIdx === -1) return c;
    c = REFLECTOR[reflectorIdx];

    for (let i = 0; i < this.rotors.length; i++) {
      c = this.rotors[i].backward(c);
    }

    return c;
  }
  process(text) {
    return text
      .toUpperCase()
      .split('')
      .map((c) => this.encryptChar(c))
      .join('');
  }
}

function promptEnigma() {
  const rl = readline.createInterface({
    input: process.stdin,
    output: process.stdout,
  });
  rl.question('Enter message: ', (message) => {
    rl.question('Rotor positions (e.g. 0 0 0): ', (posStr) => {
      const rotorPositions = posStr.split(' ').map(Number);
      rl.question('Ring settings (e.g. 0 0 0): ', (ringStr) => {
        const ringSettings = ringStr.split(' ').map(Number);
        rl.question('Plugboard pairs (e.g. AB CD): ', (plugStr) => {
          const plugPairs =
            plugStr
              .toUpperCase()
              .match(/([A-Z]{2})/g)
              ?.map((pair) => [pair[0], pair[1]]) || [];

          const enigma = new Enigma(
            [0, 1, 2],
            rotorPositions,
            ringSettings,
            plugPairs,
          );
          const result = enigma.process(message);
          console.log('Output:', result);
          rl.close();
        });
      });
    });
  });
}

if (require.main === module) {
  promptEnigma();
}

module.exports = { Enigma, Rotor };
